...
def list_assets(
    ...
    search: Optional[str] = Query(None, min_length=2, description="Buscar en nombre/hostname"),
    current_user: dict = Depends(get_current_user),
    session: Session = Depends(get_session),
):
    """
    ...
    # Aplicar filtros opcionales, validando valores
    if asset_type:
        try:
    ...
    if search:
        ...
        query = query.where(
            or_(
                col(Asset.name).ilike(search_pattern),
                col(Asset.hostname).ilike(search_pattern)
            )
        )
...
def get_asset_statistics(
    ...
    total_assets = session.exec(
        select(func.count(col(Asset.id))).select_from(query.subquery())
    ).one()

    # Agrupar por tipo
    by_type_query = (
        select(Asset.asset_type, func.count(col(Asset.id)))
    ...
    # Agrupar por estado
    by_status_query = (
        select(Asset.status, func.count(col(Asset.id)))
        .select_from(query.subquery())
    ...
    by_risk_query = (
        select(Asset.risk_level, func.count(col(Asset.id)))
        .select_from(query.subquery())
    ...
    critical_assets = session.exec(
        select(func.count(col(Asset.id))).select_from(critical_query.subquery())
    ).one()
...
@router.post("/", response_model=AssetOut, status_code=http_status.HTTP_201_CREATED)
def create_asset(
    ...
    # Crear activo
    assert owner_id is not None, "owner_id defined"
