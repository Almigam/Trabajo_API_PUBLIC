...
def list_vulnerabilities(
    ...
    if search:
        ...
        query = query.where(
            or_(
                col(Vulnerability.cve_id).ilike(search_pattern),
                col(Vulnerability.title).ilike(search_pattern) # Consulta SQL, donde se define, que busque la cadena introducida por el usuario, ignorando que sea 
...
def link_vulnerability_to_asset(
    ...
    if not vuln:
        raise HTTPException(404, "Vulnerability not found") # Si no existe la vulnerabilidad, lanzamos una excepcion HTTP, informando al usuario del error
...
def get_asset_vulnerabilities( 
    ...
    # Mapeo de severidad a valores numéricos para ordenar
    severity_order = case(
        (col(Vulnerability.severity) == RiskLevel.CRITICAL, 4),
        (col(Vulnerability.severity) == RiskLevel.HIGH, 3),
        (col(Vulnerability.severity) == RiskLevel.MEDIUM, 2),
        (col(Vulnerability.severity) == RiskLevel.LOW, 1),
        else_=0
    )
